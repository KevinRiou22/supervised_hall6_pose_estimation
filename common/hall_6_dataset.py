...
class Human36mCamera(MocapDataset):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_params) ###深度复制，防止_cameras改变影响h36m中的值 ###camera前的_代表他是private
        self._skeleton = h36m_skeleton
        #for subject, cameras in self._cameras.items():
        for subject_key, subject in self._cameras.items():
            for task_key, task in subject.items():
                for exemple_key, cameras in task.items():
                    for i, cam in enumerate(cameras):
                        #print(cam)
                        # print('before update', cam.keys())
                        cam.update(h36m_cameras_intrinsic_params[i])
                        # print('after update', cam.keys())
                        # time.sleep(10)
                        for k, v in cam.items():
                            if k not in ['id', 'res_w', 'res_h']:
                                cam[k] = np.array(v, dtype='float32')
                        ...
                        cam['center'] = normalize_screen_coordinates_new(cam['center'], w=cam['res_w'], h=cam['res_h']).astype('float32')
                        # cam['focal_length'] = cam['focal_length']/cam['res_w']*2
                        cam['focal_length'][0] = cam['focal_length'][0] / cam['res_w'] * 2
                        cam['focal_length'][1] = cam['focal_length'][1] / cam['res_h'] * 2
                        if 'translation' in cam:
                            cam['translation'] = torch.from_numpy(cam['translation'])#/1000 # mm to meters
                        if 'rotation' in cam:
                            cam['rotation'] = torch.FloatTensor(cam['rotation'])
                        ...
                        cam['project_to_2d_linear'] = torch.from_numpy(np.concatenate((cam['focal_length'],
                                                           ...
                                                           cam['radial_distortion'],
                                                           cam['tangential_distortion'])))
                        # print('project_to_2d_linear shape',cam['project_to_2d_linear'].shape)
                        #print(cam['project_to_2d_linear'])
        wd3d_path = './data/data_3d_h36m.npz'
        #data_mhf = np.load(wd3d_path, allow_pickle=True)['positions_3d'].item()
        # print(data_mhf.keys())
        #mydata = np.load(wd3d_path, allow_pickle=True)
        #positions_3d = mydata['positions_3d'].item()['S5']['Posing 1'][0].shape
        #print(type(positions_3d))
        ...
        #         }
        # self.remove_joints([4, 5, 9, 10, 11, 16, 20, 21, 22, 23, 24, 28, 29, 30, 31])
        # self.N_cam = len(cfg.HALL6_DATA.TRAIN_CAMERAS)
        self.N_cam = len(self.cfg.HALL6_DATA.TRAIN_CAMERAS) ##This is the nb of cams in extri/intri == 16
        self.camera_set = {}
        self.my_camera_set = {}
        for subject in self._cameras.keys():
            self.camera_set[subject] = {}
            for task in self._cameras[subject].keys():
                self.camera_set[subject][task] = {}
                for exemple in self._cameras[subject][task].keys():
                    #print('***', subject)
                    self.camera_set[subject][task][exemple] = {}
                    cameras = self._cameras[subject][task][exemple]
                    # print('self._cameras',cameras)
                    # time.sleep(10)
                    N_cam = len(cameras) ##This is the nb of cams in extri/intri == 16
                    ...
                    exi_mat = torch.zeros(N_cam, 3, 4)
                    exi_mat[:,:,:3] = temp_r
                    # exi_mat[:,:,-1] = torch.einsum('njc,nkc->nk',  -temp_t.view(N_cam, 1, 3), temp_r)
                    exi_mat[:, :, -1] = temp_t
                    self.camera_set[subject][task][exemple]['exi_mat'] = exi_mat
                    # print('self camera set',(self.camera_set[subject]['exi_mat']).shape)
                    # print('extri my config', exi_mat)
                    # time.sleep(10)
                    #####camera->world
                    exi_mat_inv = torch.zeros(N_cam, 3, 4)
                    exi_mat_inv[:,:,:3] = temp_r.permute(0, 2, 1)
                    exi_mat_inv[:,:,-1] = -1*temp_t.view(N_cam,3)
                    self.camera_set[subject][task][exemple]['exi_mat_inv'] = exi_mat_inv
                    ...
                    K[:,1,2] = temp_cam[0,:,0,3]
                    K[:,2,2] = 1
                    self.camera_set[subject][task][exemple]['K'] = K
                    self.camera_set[subject][task][exemple]['intrin'] = temp_cam.squeeze(0).squeeze(1)
                    ######world->image
                    prj_mat = torch.einsum('nkj,njc->nkc', K, exi_mat)#(N_view, 3, 4)
                    self.camera_set[subject][task][exemple]['prj_mat'] = prj_mat
    ...
    def p2d_cam3d_batch(self, p2d, subject_list, view_list, debug=False, extri=None, proj=None, is_predicted_params=True, confidences=None):
        ...
        for inx, sub in enumerate(subject_list):
            if self.cfg.TRAIN.LEARN_CAM_PARM:
                ...
                ex_tsk = sub[1].split("_")
                task = ex_tsk[0][4:]
                example = ex_tsk[1][7:]
                prj_mat[inx] = self.camera_set[sub[0]]["T" + str(task)]["E" + str(example)]['prj_mat']
                exi_mat[inx] = self.camera_set[sub[0]]["T" + str(task)]["E" + str(example)]['exi_mat']
        ...
        trj_c3d = torch.einsum('mnkc,mjc->mnjk', exi_mat[:,:, ...], trj_w3d_homo) #(B*T, N, J, 3)
        trj_c3d = trj_c3d.view(B, T, N, J, 3).permute(0, 1, 3, 4, 2).contiguous() ##B, T, J, 3, N)
        trj_c3d = trj_c3d - trj_c3d[:,:,:1]
    ...
    def p2d_cam3d_batch_with_root(self, p2d, subject_list, view_list, debug=False, extri=None, proj=None, is_predicted_params=True, confidences=None):
        ...
        if self.cfg.TRAIN.LEARN_CAM_PARM:
            ...
            prj_mat = torch.zeros(B, len(view_list), 3, 4)
        ...
        for inx, sub in enumerate(subject_list):
            if self.cfg.TRAIN.LEARN_CAM_PARM:
                ...
                task = ex_tsk[0][4:]
                example = ex_tsk[1][7:]
                prj_mat[inx] = self.camera_set[sub[0]]["T" + str(task)]["E" + str(example)]['prj_mat'][view_list]
                exi_mat[inx] = self.camera_set[sub[0]]["T" + str(task)]["E" + str(example)]['exi_mat'][view_list]
    ...
    def p3dcam_3dwd_batch(self, p3d, subject, view_list, extri_inv=None, is_predicted_params=True):
        ...
        p3d = p3d[..., self.cfg.HALL6_DATA.TRAIN_CAMERAS]
    ...
    def p3d_im2d_batch(self, p3d, subject, view_list, with_distor=False, flip=[], gt_2d=None):
        ...
        B, T, J, C, N = p3d.shape
        if not with_distor:
            ex_tsk = subject[0][1].split("_")
            task = ex_tsk[0][4:]
            example = ex_tsk[1][7:]
            K = self.camera_set[subject[0][0]]["T" + str(task)]["E" + str(example)]['K'][view_list]
            ...
            p_2d = p_2d.unsqueeze(2).permute(0, 3, 2, 1, 4).contiguous()
        else:
            ...
            task = ex_tsk[0][4:]
            example = ex_tsk[1][7:]
            intri_mat_ori = self.camera_set[subject[0][0]]["T" + str(task)]["E" + str(example)]['intrin'][view_list]
    ...
    def recover_extri_extri_inv_predicted_params(self, params, subjects, reduced_params=False, view_list=None):
        """
        Args:
            params: B, T, 12, N
        Returns:
        ...
        K_batch = []
        for subject in subjects:
            ex_tsk = subject[1].split("_")
            task = ex_tsk[0][4:]
            example = ex_tsk[1][7:]
            #print(self.camera_set[subject[0]]["T"+str(task)])
            K_batch.append(torch.unsqueeze(self.camera_set[subject[0]]["T"+str(task)]["E"+str(example)]['K'], 0))
        K_batch = torch.cat(K_batch, 0)[:, view_list]
        ######world->image
