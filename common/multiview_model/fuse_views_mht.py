...
class Pose3dShrink_out(Pose3dShrinkOther):
        ...
        def forward(self, x):
            ...
            x = x.view(B * N, T, -1)
            if self.cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT != "None":
                x, x_full = self.head(x)
                x_full = x_full.view(-1, N, T, self.num_joints, 3)
                x_full = x_full.permute(0, 2, 3, 4, 1)
            else:
                x = self.head(x)
            B, T, _, _ = x.shape
...
class fuse_views_mht(nn.Module):
    def __init__(self, cfg, dropout=0.1, momentum=0.1, is_train=False, num_joints=17, ):
        ...
        if self.cfg.TRAIN.LEARN_CAM_PARM==True:
            ...
            # self.learn_t.requires_grad = True
            self.parameters_shrink = Parameters3dShrink(cfg, N=N, channels=512,
                                                 dropout=cfg.NETWORK.DROPOUT, dim_joint=3, is_train=is_train,
                                                 num_joints=num_joints)
    ...
    def forward(self, pos_2d, rotation=None):
        ...
        if self.cfg.NETWORK.USE_MFT:
            ...
            if self.cfg.TRAIN.LEARN_CAM_PARM:
                params = self.parameters_shrink(f)
            if self.cfg.NETWORK.USE_FEATURE_TRAN and self.cfg.NETWORK.M_FORMER.MODE == 'mtf' and self.training:
                ...
                rot = None
        if self.cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT != "None":
            out, out_full = self.shrink_out(f)
        else:
            out = self.shrink_out(f)

        if self.training and self.use_inter_loss:
            if self.cfg.TRAIN.LEARN_CAM_PARM:
                # if self.cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT is None:
                #     return out, [out_1, out_1], params if self.cfg.NETWORK.USE_MFT else [out_1], tran, rot, params
                # else:
                print("in fuse_views : "+str(self.cfg.NETWORK.USE_MFT))
                print(out_1.shape)
                print(out.shape)
                print(params.shape)
                print(len((out, params )))
                return out, [out_1, out_1], params if self.cfg.NETWORK.USE_MFT else [out_1], tran, rot, params
            else:
                if self.cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT == "None":
                    return out, [out_1, out_1] if self.cfg.NETWORK.USE_MFT else [out_1], tran, rot
                else:
                    return out, out_full, [out_1, out_1] if self.cfg.NETWORK.USE_MFT else [out_1], tran, rot
        else:
            if self.cfg.TRAIN.LEARN_CAM_PARM:
                return out, [f_fuse_before]
            else:
                return out, [f_fuse_before], params
