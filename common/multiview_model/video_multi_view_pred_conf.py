...
class VAL(nn.Module):
    def __init__(self, cfg, in_channels, channels, dropout = 0.25, momentum = 0.1,T = 9, is_train = False, num_joints = 17):
        ...
        if cfg.DATA.DATASET_NAME == "h36m":
            ...
            self.num_view = len(cfg.HALL6_DATA.TRAIN_CAMERAS) + cfg.TRAIN.NUM_AUGMENT_VIEWS
...
class Parameters3dShrink(nn.Module):
    def __init__(self, cfg, N, channels, dropout=0.25, momentum=0.1, dim_joint=3, is_train=False, num_joints=17):
        super().__init__()
        self.conv_1 = nn.Conv2d(channels, channels, (1, 1), stride=(1, 1), dilation=(3 ** 0, 1),
                                bias=False)
        ...
        self.drop = nn.Dropout(dropout)
        self.num_layers = 0
        self.SET_COORD_SYS_IN_CAM = cfg.TRAIN.SET_COORD_SYS_IN_CAM
        conv_layers = []
        bn_layers = []
        for i in range(self.num_layers):
            if is_train:
                ...
                conv_layers.append(
                    nn.Conv2d(channels, channels, (3, 1), stride=(1, 1), dilation=(3 ** (i + 1), 1), bias=False))
            bn_layers.append(nn.BatchNorm2d(channels, momentum=momentum))
            conv_layers.append(nn.Conv2d(channels, channels, 1, bias=False))
            bn_layers.append(nn.BatchNorm2d(channels, momentum=momentum))
        self.conv_layers = nn.ModuleList(conv_layers)
        ...
        if cfg.TRAIN.PREDICT_REDUCED_PARAMETERS:
            self.shrink = nn.Conv2d(channels, 7, 1)
        else:
            self.shrink = nn.Conv2d(channels, 12, 1)
        self.dim_joint = dim_joint
        self.num_joints = num_joints
        self.pred_reduce_params = cfg.TRAIN.PREDICT_REDUCED_PARAMETERS
    def set_bn_momentum(self, momentum):
        self.bn_1.momentum = momentum
        for bn in self.bn_layers:
            bn.momentum = momentum
    ...
    def forward(self, x):
        ...
        x = self.shrink(x)
        if self.pred_reduce_params:
            x = x.view(x.shape[0], 7, -1, N)
            #torch.cat([torch.ones((1,)), torch.zeros((3,)])
            #x[..., -1] = 0
            if self.SET_COORD_SYS_IN_CAM:
                x[:, 0, :, -1] = 1
                x[:, 1:, :, -1] = 0
        else:
            x = x.view(x.shape[0], 12, -1, N)
            if self.SET_COORD_SYS_IN_CAM:
                id_transf = torch.cat([torch.eye(3, 3), torch.zeros(3, 1)], dim=-1)
                id_transf = torch.unsqueeze(id_transf, 0)
                id_transf = id_transf.repeat((x.shape[0], 1, 1))
                id_transf= torch.unsqueeze(id_transf, -1)
                x[:, :, :, -1:] = id_transf.view(x.shape[0], 12, -1, 1)
        x = x.permute(0, 2, 1, 3).contiguous()  # (B, T, J, C, N)
        return x
...
class VideoMultiViewModel(nn.Module):
    def __init__(self, cfg, momentum = 0.1,is_train = False,  num_joints = 17, ):
        ...
        self.pre_embedding = pre_proj(cfg, in_N=num_joints, h_N=N_before_mhf, dropout=cfg.NETWORK.DROPOUT, is_train=is_train)
        ...
        self.shrink_base = BERT(cfg, T=cfg.NETWORK.TEMPORAL_LENGTH, dropout=0.1, num_joints=num_joints)
        ...
        self.shrink_conf = Pose3dShrinkOther(cfg, N=N, channels=cfg.NETWORK.NUM_CHANNELS, dropout=cfg.NETWORK.DROPOUT,
                                          dim_joint=1, is_train=is_train, num_joints=num_joints)
        ...
        if self.cfg.TRAIN.LEARN_CAM_PARM==True:
            ...
            # self.learn_r = torch.nn.Parameter(torch.randn(N_Sub, N_cam, 3, 3))
            # self.learn_t = torch.nn.Parameter(torch.randn(N_Sub, N_cam, 3))
            # self.learn_r.requires_grad = True
            # self.learn_t.requires_grad = True
            #
            ...
            # N_Sub = len(Sub_hub)
            # N_cam = len(self.cfg.H36M_DATA.TRAIN_CAMERAS)
            # self.learn_theta = torch.nn.Parameter(torch.randn(N_Sub, N_cam, 3))
            # self.learn_t = torch.nn.Parameter(torch.randn(N_Sub, N_cam, 3))
            # self.learn_theta.requires_grad = True
            # self.learn_t.requires_grad = True
            self.parameters_shrink = Parameters3dShrink(cfg, N=N, channels=cfg.NETWORK.NUM_CHANNELS,
    ...
    def set_bn_momentum(self, momentum):
        ...
        self.shrink_base.set_bn_momentum(momentum)
        self.shrink_conf.set_bn_momentum(momentum)
    ...
    def forward(self, pos_2d, rotation = None):
        ...
        f = self.pre_embedding(pos_2d) #(B, K, D, T, N)
        ...
        if self.cfg.NETWORK.USE_MFT:
            ...
            if self.cfg.NETWORK.USE_FEATURE_TRAN and self.cfg.NETWORK.M_FORMER.MODE == 'mtf' and self.training:
                ...
                rot = None
        out = torch.sigmoid(self.shrink_conf(f))
