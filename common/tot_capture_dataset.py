...
class Human36mCamera(MocapDataset):
    def __init__(self, cfg): 
        ...
        params = np.load("./data/cam_param_full_resolution_mpii_train.npz", allow_pickle=True)
        params = params['cam_param']
        self.camera_set = {}
        for subject in self.cfg.H36M_DATA.SUBJECTS_TEST:#self._cameras.keys():
            print('***', subject)
            ...
            K = torch.zeros(8, 3, 3)
            for view in range(8):
                K[view, 0, 0] = params.item()['S{}'.format(1)]["Action1"][view][0]["fx"]#temp_cam[0,:,0,0]
                K[view,1,1] = params.item()['S{}'.format(1)]["Action1"][view][0]["fy"]#temp_cam[0,:,0,1]
                K[view,0,2] = params.item()['S{}'.format(1)]["Action1"][view][0]["cx"]#temp_cam[0,:,0,2]
                K[view,1,2] = params.item()['S{}'.format(1)]["Action1"][view][0]["cy"]#temp_cam[0,:,0,3]
                K[view,2,2] = 1
    ...
    def p2d_cam3d(self, p2d, subject, view_list, debug=False):
        ...
        trj_w3d_homo = torch.cat((trj_w3d, torch.ones(trj_w3d.shape[0], 16, 1)), dim = -1)
    ...
    def p2d_cam3d_batch(self, p2d, subject_list, view_list, debug=False, extri=None, proj=None, is_predicted_params=True):
        ...
        trj_w3d = triangulation_torch(p2d.view(-1, J, C, N).permute(0, 1, 3, 2).contiguous().to(p2d.device), prj_mat[:, view_list,...].to(p2d.device)) #(B*T, J, 3)
        trj_w3d_homo = torch.cat((trj_w3d, torch.ones(trj_w3d.shape[0], 16, 1).to(p2d.device)), dim = -1)
        trj_c3d = torch.einsum('mnkc,mjc->mnjk', exi_mat[:,view_list, ...].to(p2d.device).detach(), trj_w3d_homo) #(B*T, N, J, 3)
        trj_c3d = trj_c3d.view(B, T, N, J, 3).permute(0, 1, 3, 4, 2).contiguous() ##B, T, J, 3, N)
        trj_c3d = trj_c3d - trj_c3d[:,:,:1]
        return trj_c3d
    ...
    def recover_extri_extri_inv_predicted_params(self, params, subjects, views_list):
        """
        Args:
            params: B, T, 12, N
        Returns:
        ...
        """
        learn_theta = params[:, :3, :].permute(0, 2, 1).contiguous()
        # print("learn_theta.shape : " + str(learn_theta.shape))
        learn_t = params[:, 3:, :].permute(0, 2, 1).contiguous()
        # print("learn_t.shape : " + str(learn_theta.shape))
        # input()
        learn_r, learn_t = self.recover_theta_to_extri(learn_theta, learn_t)
        """
        learn_r = params.view(B, 3, -1, N)[:, :, :3].permute(0, 3, 1, 2)
        learn_t = params.view(B, 3, -1, N)[:, :, 3:].permute(0, 3, 1, 2)
        idx = 0
        all_extri = []
        all_extri_inv = []
        all_proj = []
        ...
        for subject in subjects:
            ...
            #####camera->world
            exi_mat_inv = torch.zeros(N_cam, 3, 4)
            exi_mat_inv[:, :, :3] = learn_r[idx].permute(0, 2, 1)
            ...
            ######world->image
            K = self.camera_set[subject[0]]['K'][views_list]
            prj_mat = torch.einsum('nkj,njc->nkc', K, exi_mat)  # (N_view, 3, 4)
    ...
    def p3dcam_3dwd_batch(self, p3d, subject, view_list, extri_inv=None, is_predicted_params=False):
    ...
    def p3dwd_p2dim_batch(self, p3d, subject, view_list, exi_mat=None):
        ...
        for inx, sub in enumerate(subject):
            prj_mat[inx] = self.camera_set[sub[0]]['prj_mat']
        p_3d_homo = torch.cat((p3d, torch.ones(p3d.shape[0], 16, 1).to(p3d.device)), dim = -1)
