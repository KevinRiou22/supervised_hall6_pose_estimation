    ...
    for task in tasks:
        for example in examples:
            ...
            if 'task{}_example{}'.format(task, example) in keypoints_new['S{}'.format(sub)]:
                ...
                for id, i in enumerate(cfg.HALL6_DATA.TRAIN_SAMPLE_CAMERAS):
...
if True:
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            # prepare a view_list by randomly sampling 4 cams from TRAIN_SAMPLE_CAMERAS
            print(inputs_3d_gt.shape)
            view_list = np.random.choice(cfg.HALL6_DATA.TRAIN_SAMPLE_CAMERAS, 4, replace=False)
            print(view_list)
            N = len(view_list)
            ...
            print('Total Loss is {}'.format(loss_total))
            loss_total.backward()
            optimizer.step()
            iters += 1
        process.close()
